...
static bool performStartupChecks(char *reason, size_t len) {
  ...
  // In serial-simulator mode, avoid touching real SPI sensors — assume they
  // will be simulated via terminal. Provide safe default readings so startup
  // checks pass: set to active program setpoints.
  (void)xSemaphoreSPI;
  float t1 = recipes[sysStatus.active_program_idx].h1_setpoint_c;
  float t2 = recipes[sysStatus.active_program_idx].h2_setpoint_c;
  (void)t1;
  (void)t2; // used only to avoid unused warning if needed
#else
  ...
  int criticalPins[] = {PIN_SSR_1, PIN_SSR_2, PIN_MOTOR_DOWN, PIN_MOTOR_UP,
                        PIN_VSPI_SCK};
...
bool g_benchTempSimEnabled =
    false; // Disabled by default; enabled via Virtual TFT for bench testing
...
const char *getTorqueUnitName(TorqueUnit_t unit) {
  switch (unit) {
  case TORQUE_UNIT_KG_CM:
    return "kg.cm";
  case TORQUE_UNIT_LB_IN:
    return "lb.in";
  case TORQUE_UNIT_NM:
  default:
    return "Nm";
...
float getTorqueConversionFactor(TorqueUnit_t unit) {
  switch (unit) {
  case TORQUE_UNIT_KG_CM:
    return 10.19716f; // 1 Nm = 10.19716 kgf.cm
  case TORQUE_UNIT_LB_IN:
    return 8.85075f; // 1 Nm = 8.85075 lbf.in
  case TORQUE_UNIT_NM:
  default:
    return 1.0f;
  }
...
void setup() {
  ...
  // Initialize MAX31865 RTD sensors in Continuous Conversion mode with 50Hz
  // mains filter
...
#if ENABLE_UART_VIRTUAL_TFT || ENABLE_SERIAL_TFT || INPUT_USE_SERIAL
  // Print a concise boot summary of enabled/disabled modules to Serial (useful
  // for testing)
  Serial.println("==================================================");
  ...
  DEBUG_PRINTF(
      "[BOOT] Relay: %s, Torque Unit: %s, Temp Manip: H1=%.1f%%, H2=%.1f%%\n",
      (g_relayType == RELAY_TYPE_SSR ? "SSR" : "NORMAL"),
      getTorqueUnitName(g_torqueUnit), g_h1_temp_manip_pct,
      g_h2_temp_manip_pct);
  ...
  xTaskCreatePinnedToCore(Task_SafetyAndControl, "SafetyTask", 4096, NULL, 3,
                          NULL, 1);
  xTaskCreatePinnedToCore(Task_TemperaturePID, "PIDTask", 4096, NULL, 2, NULL,
                          1);
