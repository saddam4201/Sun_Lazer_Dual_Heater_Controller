...
class VirtualTFTApp(tk.Tk):
    ...
    def demo_simulation_worker(self):
        ...
        try:
            ...
            self.cmd_queue.put("RRECT,6,26,150,84,4,0x4A69,0")
            self.cmd_queue.put("TXT,14,30,2,0x07FF,0x0000,HEATER 1")
            self.cmd_queue.put("TXT,14,88,2,0xC618,0x0000,SET: 50.0 C")

            # Card 2: Heater 2 (y = 26..110, h = 84)
            self.cmd_queue.put("RRECT,164,26,150,84,4,0x4A69,0")
            self.cmd_queue.put("TXT,172,30,2,0x07FF,0x0000,HEATER 2")
            self.cmd_queue.put("TXT,172,88,2,0xC618,0x0000,SET: 50.0 C")

            # Card 3: Torque (y = 116..200, h = 84)
            self.cmd_queue.put("RRECT,6,116,150,84,4,0x4A69,0")
            self.cmd_queue.put("TXT,14,120,2,0xFFE0,0x0000,TORQUE")

            # Card 4: Timer (y = 116..200, h = 84)
            self.cmd_queue.put("RRECT,164,116,150,84,4,0x4A69,0")
            self.cmd_queue.put("TXT,172,120,2,0x07E0,0x0000,TIMER")
            ...
            while not self.sim_stop_event.is_set():
                ...
                self.cmd_queue.put(f"TXT,14,52,4,0xFFFF,0x0000,{h1_act:.1f}")
                self.cmd_queue.put("TXT,105,54,2,0xFFFF,0x0000,C")

                self.cmd_queue.put(f"TXT,172,52,4,0xFFFF,0x0000,{h2_act:.1f}")
                self.cmd_queue.put("TXT,265,54,2,0xFFFF,0x0000,C")

                self.cmd_queue.put(f"TXT,14,148,4,0xFFFF,0x0000,{torque:.2f}")
                self.cmd_queue.put("TXT,105,150,2,0xFFFF,0x0000,Nm")

                mm = timer_sec // 60
                ss = timer_sec % 60
                self.cmd_queue.put(f"TXT,172,148,4,0x07E0,0x0000,{mm:02d}:{ss:02d}")
